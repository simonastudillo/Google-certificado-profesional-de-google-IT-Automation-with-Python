...
## Guía de estudio: Expresiones y variables
- Términos:
    - expression (expresión): Una combinación de números, símbolos y variables que representan un valor y retornan un resultado.
    - data types (Tipos de datos): Diferentes tipos de valores que pueden ser almacenados en variables, como enteros, cadenas de texto y números decimales.
    - variable (variable): Una instancia de un tipo de dato, representada por un nombre único, este almacena variables variables.
    - implicit conversion (conversión implícita): Cuando Python convierte automáticamente un tipo de dato en otro cuando es necesario.
    - explicit conversion (conversión explícita): Cuando convertimos un tipo de dato en otro usando funciones como int(), float() y str().

- Anotaciones de variables: Una forma de indicar el tipo de dato que se espera que tenga una variable, usando el símbolo de dos puntos (:) seguido del tipo de dato esperado después del nombre de la variable.
```Python
import typing
...
- Habilidades
    ...
    - Usa aritmetica básica con variables para crear expresiones
    - Usa conversión explicita para cambiar el de dato de `float` a `string`
```Python
...
```
    - Retorna multiples variables de string en una osla linea formando una oración
    - Usa el operador más (+) para concatenar strings y variables
    - Agrega espacios entre las variables en la función `print()` para que la oración sea legible
```Python
...
```
    - Resuelve el TypeError causado por el tipo de dato
    - Usa la conversión explicita
```Python
...
```
    - Resuelve un error de división por cero (`ZeroDivisionError`)`
```Python
numerator = 7
